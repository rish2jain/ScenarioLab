    ...
    try:
        ...
        if report.risk_register and report.risk_register.items:
            ...
            for risk in report.risk_register.items:
                ...
                desc = risk.description[:50] + "..." \
                    if len(risk.description) > 50 else risk.description
                lines.append(
                    f"| {risk.risk_id} | {desc} | "
        ...
        # Stakeholder Heatmap
        if (report.stakeholder_heatmap and
                report.stakeholder_heatmap.stakeholders):
            lines.extend([
                "## Stakeholder Heatmap",
                "",
        ...
        if report.checkpoints:
            ...
            for cp in report.checkpoints:
                ...
                lines.append(
                    f"| {cp.checkpoint_id} | {cp.stage} | "
                    f"{status_emoji} {cp.status} | {notes}... |"
                )
