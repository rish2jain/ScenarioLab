...
def generate_report_html(report: SimulationReport) -> str:
    ...
    html_parts = [
        ...
        "        .info-box { background: #ecf0f1; padding: 15px; "
    ...
    if report.executive_summary:
        summary = report.executive_summary.summary_text.replace(
            chr(10), '<br>'
        )
        html_parts.extend([
    ...
    if report.scenario_matrix and report.scenario_matrix.scenarios:
        html_parts.extend([
            ...
            "    <table>",
            "        <tr><th>Scenario</th><th>Probability</th>"
            "<th>Key Drivers</th></tr>",
        ])
    ...
    html_parts.extend([
        "    <div class=\"footer\">",
        f"        <p>Report generated by MiroFish on "
        f"{datetime.utcnow().isoformat()}</p>",
        "    </div>",
        "</body>",
        "</html>",
    ])
