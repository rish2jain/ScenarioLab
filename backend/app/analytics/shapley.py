...
class ShapleyAnalyzer:
    ...
    async def compute_attribution(
        ...
        logger.info(
            f"Computing attribution for simulation {simulation_id} "
        ...
        agent_attributions = await self._compute_agent_attributions(
            agents, all_messages, all_decisions, outcome_metric
        )
    ...
    async def _compute_agent_attributions(
        ...
        if self.llm:
            for agent in agents:
                ...
                influence_result = await self._assess_agent_influence(
                    agent, agent_msgs, decisions, outcome_metric
                )
                ...
                attributions.append(AgentAttribution(
                    ...
                    attribution_score=influence_result.get(
                        "attribution", 0.5
                    ),
                    confidence_interval=(round(ci_low, 3), round(ci_high, 3)),
                    key_contributions=influence_result.get(
                        "contributions", []
                    ),
                ))
        else:
        ...
        if total > 0:
            for attr in attributions:
                attr.attribution_score = round(
                    attr.attribution_score / total, 4
                )
    ...
    async def _assess_agent_influence(
        ...
        msg_summary = "\n".join([
            f"Round {m.round_number}: {m.content[:200]}..."
            for m in messages[-10:]  # Last 10 messages
        ])

        # Prepare decision summary
        decision_summary = "\n".join([
            json.dumps(d, indent=2)[:300]
...
Respond with valid JSON only."""
            ...
            response = await self.llm.generate(
                messages=[
                    LLMMessage(
                        role="system",
                        content=(
                            "You analyze agent contributions to group "
                            "outcomes. Respond with valid JSON only."
                        ),
                    ),
    ...
    async def detect_coalitions(
        ...
        for msg in all_messages:
            ...
            support_words = [
                "agree with", "support", "align with",
                "same position", "concur",
            ]
                    ...
                    for other in agents:
                        if other.id != msg.agent_id:
                            ...
                            if name_lower in content_lower:
                                pair = tuple(
                                    sorted([msg.agent_id, other.id])
                                )
                                alignment_scores[pair] += 0.3
                                interaction_counts[pair] += 1

        # Form coalitions from high-alignment pairs
        coalition_groups = self._form_coalition_groups(
            agents, alignment_scores, interaction_counts
        )
        ...
        for i, group in enumerate(coalition_groups):
            member_names = [
                a.name for a in agents if a.id in group
            ]
            ...
            interactions = sum(
                c for pair, c in interaction_counts.items()
                if pair[0] in group or pair[1] in group
            )
            if interactions > 0:
                coalition_score /= interactions

            coalitions.append(CoalitionAttribution(
                coalition_id=f"coalition_{i + 1}",
                members=list(group),
                member_names=member_names,
                attribution_score=round(min(coalition_score, 1.0), 3),
                key_influence=self._describe_coalition_influence(
                    group, member_names
                ),
            ))
