...
export interface SetParametersProps {
  w: SimulationWizardState;
}

export function SetParameters({ w }: SetParametersProps) {
  const {
    simulationName,
    setSimulationName,
    hybridAvailable,
  ...
  const modelValues = ['', ...wizardLlmModels.map((m) => m.id)];
  const modelTabValue = modelValues.includes(modelSelection)
    ? modelSelection
    : '';
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-foreground">Set Parameters</h2>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-sm text-foreground-muted mt-1">
                  Round 1 uses your cloud provider for quality calibration. Subsequent rounds
                  run locally.
                </p>
              </div>
            </label>
            ...
            {Array.isArray(evidencePacks) && evidencePacks.length > 0 ? (
              <ul className="text-xs text-foreground-muted space-y-1 max-h-32 overflow-y-auto">
                {evidencePacks.map((p, i) => {
                  ...
                  const citationCount = Array.isArray(p.citations) ? p.citations.length : 0;
                  return (
                    <li key={i}>
        ...
        <div>
          ...
          >
            <button
              type="button"
              role="radio"
              aria-checked={modelSelection === ''}
              tabIndex={rovingTabIndex(modelTabValue === '')}
